...
import { GameStatus, UserRole, GameType } from '@prisma/client';
...
export async function createGame(data: {
	...
	try {
		...
		if (data.intervalTime < 5 || data.intervalTime > 900) {
			return {
				success: false,
				message: 'Invalid interval time duration',
			};
		}
		...
		// Get first question to determine category and difficulty
		const firstQuestion = await db.question.findUnique({
			where: { id: data.questions[0] },
			select: { category: true, difficulty: true },
		});

		// Create game with custom durations and new fields
		...
		const cycleDuration =
			data.answerTime + data.resultTime + data.intervalTime;
...
export async function submitAnswer(data: {
	...
	try {
		...
		const speedBonus = isCorrect
			? (timeLeft / gameQuestion.duration) * 10
			: 0;
		...
		return {
			...
			data: {
				...
				currentStatus: {
					...
					currentScore: game.responses.reduce(
						(sum, r) => sum + r.score,
						0
					),
				},
