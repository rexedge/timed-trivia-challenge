...
interface GameContainerProps {
	...
	onGameComplete?: (finalScore: number) => Promise<void>;
}
...
export function GameContainer({
	...
	const calculatePhaseEndTime = (
		...
		switch (phase) {
			case GamePhase.QUESTION:
				return new Date(
					questionDisplayTime.getTime() + game.answerTime * 1000
				);
		...
		for (let i = 0; i < questions.length; i++) {
			...
			if (now >= displayTime && now < nextDisplayTime) {
				...
				if (isAnswered) {
					...
					const timeSinceAnswer =
						now.getTime() - answerTime.getTime();

					if (timeSinceAnswer < gameData.game.resultTime * 1000) {
						currentPhase = GamePhase.RESULT;
						phaseEnd = new Date(
							answerTime.getTime() +
								gameData.game.resultTime * 1000
						);
						// Set result...
					} else {
		...
		const initializeAndUpdate = async () => {
			try {
				...
				if (result.success) {
					...
					const { currentIndex, currentPhase, phaseEnd } =
						initializeGameState(result.data.game.gameQuestions);
					...
					if (result.data.game.status === GameStatus.COMPLETED) {
						if (onGameComplete) {
							await onGameComplete(
								result.data.currentStatus.currentScore
							);
						} else {
	...
	const handleSubmit = async (answer: string) => {
		...
		try {
			...
			setPhaseEndTime(
				calculatePhaseEndTime(GamePhase.RESULT, new Date())
			);
		...
		return (
			<div className='text-center py-8'>
				...
				<p className='text-muted-foreground mb-4'>
					Thank you for participating! Check the leaderboard for final
					results.
				</p>
				<button
					...
					<Card>
						...
						<CardContent>
							<p className='text-muted-foreground'>
								The next question will be available soon. Check
								the leaderboard to see your current ranking!
							</p>
						</CardContent>
					</Card>
				)}
